...
class BackgroundImageGenerator:
    ...
    def __init__(self, topic=None):
        ...
        # 如果有主题，创建主题目录
        if self.topic:
            self.theme_dir = os.path.join(self.config['output_dir'],
                                          self.topic)
    ...
    def generate(self, title_text, **kwargs):
        ...
        image = Image.new('RGB', (self.width, self.height),
                          kwargs.get('bg_color', self.background_color))
        ...
        title_lines = textwrap.wrap(
            title_text, width=config['title_max_width'])
        ...
        if config.get('subtitle_lines'):
            ...
            for i, line in enumerate(config['subtitle_lines']):
                ...
                x_offset = self.width - bbox[2] - (config['padding'] + 30) + (
        ...
        line_y = self.height - config['padding'] - config[
            'line_position_offset']
        draw.line([(0, line_y), (self.width, line_y)],
                  fill=(0, 0, 0),
        ...
        output_path = os.path.join(output_dir, f'title_{uuid.uuid4()}.png')
