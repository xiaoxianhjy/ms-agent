...
from optimized_manim_prompts import FixContext, OptimizedManimPrompts
    ...
    def process_manim_code(self, raw_code, scene_name, content_description=''):
        ...
        for attempt in range(1, self.max_fix_attempts + 1):
            ...
            try:
                ...
                # 清理并验证修复结果
                cleaned_fixed_code = self.preprocessor.get_clean_code(
                    fixed_code)
                new_report = self.preprocessor.preprocess_code(
                    cleaned_fixed_code, f'{scene_name}_fix{attempt}')
                ...
                if improvement['improved']:
                    log.append(
                        f"   第{attempt}次修复成功，改善: {improvement['description']}")
...
'''
    ...
    def _assess_improvement(self, old_report, new_report):
        ...
        improvement = {'improved': False, 'description': '', 'resolved': []}
...
def integrate_with_workflow():
    ...
    def enhanced_code_processing(raw_code, scene_name, content_description=''):
        ...
        # 处理代码
        result = controller.process_manim_code(raw_code, scene_name,
                                               content_description)
