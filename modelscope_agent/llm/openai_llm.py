...
import inspect
from typing import Any, List, Dict, Optional, Generator, Iterable
...
from modelscope_agent.utils import retry
from modelscope_agent.utils import assert_package_exist
...
class OpenAI(LLM):
            ...
            tools = [
                {
                    'type': 'function',
                    'function': {
                        'name': f'{tool["tool_name"]}' if tool.get('server_name') else tool['tool_name'],
    ...
    def generate(self, messages: List[Message], tools: Optional[List[Tool]] = None, **kwargs) -> Message | Generator[Message, None, None]:
        """Generate response.
        ...
        if stream:
            return self._stream_continue_generate(messages, completion, tools, **args)
        else:
    ...
    def _stream_continue_generate(self, messages: List[Message], completion: Iterable, tools: Optional[List[Tool]] = None, **kwargs) -> Generator[Message, None, None]:
        ...
        for chunk in completion:
            ...
            if chunk.choices[0].finish_reason in ['length', 'null']:
                ...
                completion = self._continue_generate_recursive(messages, message, tools, **kwargs)
                for chunk in self._stream_continue_generate(messages, completion, tools, **kwargs):
                    yield chunk
    ...
    def _continue_generate_recursive(self, messages: List[Message], new_message, tools: List[Tool] = None, **kwargs):
    ...
    def _continue_generate(self, messages: List[Message], completion, tools: List[Tool] = None, **kwargs) -> Message:
        ...
        if completion.choices[0].finish_reason in ['length', 'null']:
            ...
            completion = self._continue_generate_recursive(messages, new_message, tools, **kwargs)
            return self._continue_generate(messages, completion, tools, **kwargs)
